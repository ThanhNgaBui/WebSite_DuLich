		...
		public function __construct(){
			...
			switch($act){
				case "delete":
					...
					$this->model->execute("delete from cam_nang_du_lich where pk_id=$id");
					header("location:admin.php?controller=cam_nang_du_lich");
				break;
			}
			//------------
			$so_ban_ghi_tren_1_trang = 10;
			$tong_so_ban_ghi = $this->model->num_rows("select * from cam_nang_du_lich");
			...
			$arr = $this->model->fetch("select * from cam_nang_du_lich order by pk_id desc limit $tu_ban_ghi,$so_ban_ghi_tren_1_trang");
			//load view
			include "view/backend/view_cam_nang_du_lich.php";
			//------------
		}
	}
