		...
		public function __construct(){
			...
			switch($act){
				case "delete":
					...
					$this->model->execute("delete from danh_lam_thang_canh where pk_id=$id");
					header("location:admin.php?controller=danh_lam_thang_canh");
				break;
			}
			//------------
			$so_ban_ghi_tren_1_trang = 10;
			$tong_so_ban_ghi = $this->model->num_rows("select * from danh_lam_thang_canh");
			...
			$arr = $this->model->fetch("select * from danh_lam_thang_canh order by pk_id desc limit $tu_ban_ghi,$so_ban_ghi_tren_1_trang");
			//load view
			include "view/backend/view_danh_lam_thang_canh.php";
			//------------
		}
	}
